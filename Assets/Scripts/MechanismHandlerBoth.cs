...
public class MechanismHandlerBoth : MonoBehaviour {
    ...
    public bool loading;
    public bool isTranslatingRight = false;
    public bool isTranslatingLeft = false;
    ...
    public IEnumerator PawnFallCalculation (Cell startCell, int player, bool reset, bool click, int variant) {
        if (variant == 0) {
            yield return StartCoroutine (PawnFallCalculationVariantBastien(startCell, player, reset, click));
        } else {
            yield return StartCoroutine (PawnFallCalculationVariantRomain(startCell, player, reset, click));
        }
    }
...
//---------------------------------------------------------------------------------------------//
    ...
    /// <returns></returns>
    public IEnumerator PawnFallCalculationVariantBastien (Cell startCell, int player, bool reset, bool click) {
        ...
        if (!reset) {
            ...
            yield return StartCoroutine (PawnFallDoVariantBastien (newPawn, currentCell, player, false, startCell));
        } else {
            Transform existingPawn = startCell.GetComponentInChildren<Pawn> ().transform;
            yield return StartCoroutine (PawnFallDoVariantBastien (existingPawn, currentCell, player, true, startCell));
        }
    ...
    public Cell NextCellVariantBastien (Cell currentCell, int gravity) {
    ...
    /// <param name="gravity"></param>
    /// <returns>the next cell if any, else returns calling cell</returns>
    public Cell PreviousCellVariantBastien (Cell currentCell, int gravity) {
        //0: down | 1: left | 2: up | 3: right 
        Coord currentCoordinates = currentCell.coordinates; 
        switch (gravity) {
        case 0:
    ...
    public IEnumerator PawnFallDoVariantBastien (Transform pawn, Cell endCell, int player, bool reset, Cell startCell) {
        ...
        yield return StartCoroutine (AnimateFallVariantBastien (pawn, endCell, reset));
        
        if (topCell.coordinates != endCell.coordinates && PreviousCellVariantBastien (endCell,gravity).available) {
        ...
        if (triggers.Count != 0 && !reset) {
            foreach (Cell.Trigger trigger in triggers) {
                yield return StartCoroutine (ExecuteTriggerVariantBastien (trigger.triggerType, 1.0f));
            }
        }
    ...
    /// <param name="reset"> true if called from a resetGravity trigger, else false </param>
    /// <returns></returns>
    IEnumerator AnimateFallVariantBastien (Transform pawn, Cell endCell, bool reset) {
    ...
    /// <returns></returns>
    IEnumerator ExecuteTriggerVariantBastien (int triggerType, float time) {
        int rotate = 0;
    ...
    public IEnumerator TranslationVariantBastien (Vector3 direction, float time) {
        foreach (Transform cellChild in myGrid.GetComponent<Transform> ()) {
            //coordonnées
    ...
    /// </summary>
    public void ResetGravityVariantBastien () {
        int player;
        Cell cellToReset;
            ...
            for (int y = 1; y < myGrid.gridSize.y; y++) {
                for (int x = 0; x < myGrid.gridSize.x; x++) {
                    ...
                    if (cellToReset.GetComponentInChildren<Pawn> ()) {
                        ...
                        StartCoroutine (PawnFallCalculationVariantBastien (cellToReset, player, true, false));
                    }
                }
            }
            break;
    ...
    /// </summary>
    public void CheckAlign4VariantBastien () {
...
//---------------------------------------------------------------------------------------------//
    ...
    /// <param name="reset">true if called from a resetGravity trigger, else false </param>
    /// <returns></returns>
    public IEnumerator PawnFallCalculationVariantRomain (Cell startCell, int player, bool reset, bool click) {
        // Calcule où le pion va s'arrêter de chuter depuis les coordonnées ou il a été lâché.
        Coord interCoords = startCell.coordinates;
        Cell currentCell = startCell;
        bool falling = true;
        ...
        //script plaçant le pion et lançant le visuel
        if (!reset) {
            yield return StartCoroutine (PawnFallDoVariantRomain (currentCell, player, false, startCell, click));
        } else {
    ...
    /// <param name="gravity"></param>
    /// <returns>the next cell if any, else returns calling cell</returns>
    public Cell PreviousCellVariantRomain (Cell currentCell, int gravity) {
        //0: down | 1: left | 2: up | 3: right 
        Coord currentCoordinates = currentCell.coordinates; 
        switch (gravity) {
        case 0:
    ...
    /// <param name="startCell"></param>
    /// <returns></returns>
    public IEnumerator PawnFallDoVariantRomain (Cell endCell, int player, bool reset, Cell startCell, bool click) {
        //rend la case finale non disponible pour les futurs pions
    ...
    /*    if (!reset) {
        ...
        yield return StartCoroutine (AnimateFallVariantRomain (pawn, endCell, reset));
        
        if (startCell.coordinates != endCell.coordinates && 
            PreviousCellVariantRomain (endCell,gravity).available &&
    ...
    /// <param name="reset"> true if called from a resetGravity trigger, else false </param>
    /// <returns></returns>
    IEnumerator AnimateFallVariantRomain (Transform pawn, Cell endCell, bool reset) {
        //continuous speed for now, will improve later
        float speed = 10f;
    ...
    /// <param name="time"> durée totale de l'animation </param>
    /// <returns></returns>
    IEnumerator ExecuteTriggerVariantRomain (int triggerType, float time) {
        int rotate = 0;
        float elapsedTime = 0.0f;
        switch (triggerType) {
        //gravity -> 0: down | 1: left | 2: up | 3: right
    ...
    /// Parcours la grille de bas en haut (selon la gravité actuelle) et simule un nouveau lâché de pion pour tous ceux déjà en place
    /// </summary>
    public void ResetGravityVariantRomain () {
        int player;
        Cell cellToReset;
            ...
            for (int y = 1; y < myGrid.gridSize.y; y++) {
                for (int x = 0; x < myGrid.gridSize.x; x++) {
                    ...
                    if (cellToReset.GetComponentInChildren<Pawn> ()) {
                        cellToReset.available = true;
                        player = cellToReset.GetComponentInChildren<Pawn> ().player;
                        StartCoroutine (PawnFallCalculationVariantRomain (cellToReset, player, true, false));
                    }
                }
            }
            break;
    ...
    /*public void CheckAlign4VariantRomain (Cell newFilled, int player) {
        ...
        foreach (string i in new List<string> () {"right", "UR", "up", "UL"}) {
            ...
            while(gridAtlas.gridDictionary.ContainsKey (startCoords - fallIntegers[directionConversionStringVariantRomain (i)])){
                // On a rejoint le bord du graphique, prêts à balayer en sens inverse.
                startCoords -= fallIntegers[directionConversionStringVariantRomain (i)];
            }
            int count = 0;
            ...
            while (gridAtlas.gridDictionary.ContainsKey (startCoords)) {
                if (gridAtlas.gridDictionary[startCoords].GetComponentInChildren<Pawn> () && gridAtlas.gridDictionary[startCoords].GetComponentInChildren<Pawn> ().player == player) {
                    if (count + 1 >= 4) {
                        ...
                        destCoords = originCoords - 3 * fallIntegers [directionConversionStringVariantRomain (i)];
                    ...
                    else if (!IsBlockedVariantRomain (startCoords, i)) count++;
                    else count = 0;
                }
                else count = 0;
                startCoords += fallIntegers[directionConversionStringVariantRomain (i)];
    ...
    public bool IsBlockedVariantRomain (Coord coords, string direction) {
        switch (direction) {
        case "right":
    ...
    public int directionConversionStringVariantRomain (string direction) {
        //0: down | 1: left | 2: up | 3: right | 4: upLeft | 5: upRight
        try {
